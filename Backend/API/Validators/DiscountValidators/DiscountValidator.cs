    ...
    public class DiscountValidator : AbstractValidator<Discount>
    {
        public DiscountValidator()
        {
            RuleFor(x => x.DiscountName)
                ...
                .Matches(@"^[\p{L}\p{N}]+$").WithMessage("Discount name must contain only letters and numbers.");

            RuleFor(x => x.DiscountValue)
                .NotEmpty().WithMessage("Discount value is required.")
                .InclusiveBetween(0.01f, 0.99f).WithMessage("Discount value must be between 0.01 and 0.99.");
        }
    }
}
